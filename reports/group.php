...
$query_datalist = "SELECT $columns FROM $table";
...
$rrr= substr($rrr1, 0, -2);
$rrr21= substr($rrr2, 0, -1);

$_SESSION['groups']=$rrr;
$_SESSION['groupsTotal']=$rrr21;
    ...
    <div id="wrapper">
        ...
        <div id="page-content-wrapper">
            <div class="container-fluid">
                <div class="row">
                    <div class="col-lg-12">
                        <h1 class="h1">Column Grouping</h1>
                        <h3>Group Colums from Selected Table. <?php echo $totalRows_datalist; ?> Data found at selected table.</h3>
                        <p class="text-primary">Please Select one or more colum(s) from <strong><?php echo $_SESSION['table']; ?> if you need grouping</strong>.</p>
...
<table class="table table-striped table-bordered table-hover">
                            <thead>
                                <tr>
                                    <th width="50">#</th>
                                    <th>Column Name </th>
                                    <th width="">Data Count <input type="button" name="button2" id="button2" value="Previous" class="form-control"  style="width:100px; float:right;" onClick="window.location='columns.php';"></th>
...
mysqli_select_db($sys, $database_sys);
$query_totalRecords = "SELECT * FROM ".$table." GROUP BY ".$value;
...
?>                                
                                <tr>
                                    <td><?php echo $i; ?></td>
                                    <td style="font-size:16px;"><?php echo $value; ?></td>
                                    <td><?php echo $totalRows_totalRecords; ?> <?php if($totalRows_datalist==$totalRows_totalRecords || $totalRows_totalRecords==1){?><i class=" alert-danger">Cannot Group. No Grouped Data Found !<?php }?></i></td>
                                    <td>
                                      <input <?php if($totalRows_datalist==$totalRows_totalRecords || $totalRows_totalRecords==1){ echo "disabled=\"disabled\""; }?>  class="form-control" type="radio" name="checkbox" id="checkbox<?php echo $i; ?>" value="<?php echo $value; ?>" <?php if($value==$_SESSION['groupsTotal']){ ?>checked="checked"<?php }?>>
                                  </td>
                                </tr>
<?php }?>
                                <tr>
                                  <td>&nbsp;</td>
                                  <td>&nbsp;</td>
                                  <td align="right" style="font-size:16px;"><input type="button" name="button2" id="button2" value="Previous" class="form-control"  style="width:100px;" onClick="window.location='columns.php';"></td>
